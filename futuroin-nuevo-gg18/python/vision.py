...
# --- TWO-LINE corner sequence: entry line REGISTERS, second line CONFIRMS -----
# Every WRO corner zone has BOTH colours, ~a corner-zone width apart. Driving CW
# the car meets ORANGE first then BLUE; driving CCW it meets BLUE first then
# ORANGE. Firing the 90-deg turn on the FIRST line (old behaviour) started the
# arc at the OUTER edge of the corner zone -> the car came around early and hit
# the INNER wall. Now the first (ENTRY) line only REGISTERS the corner: the car
# keeps driving forward INTO the zone, and turn() fires once the SECOND
# (confirmation) colour is in view -- i.e. the arc starts from BETWEEN the two
# lines. The CAMERA keeps direction authority: the first colour seen in the run
# latches CW/CCW, and the confirmation colour is always the opposite one.
CONFIRM_MIN_PIX   = LINE_MIN_PIX  # px of the confirmation colour to accept it
CONFIRM_MIN_S     = 0.25  # keep driving at least this long after the entry line
                          # before the turn may fire (the "go a little forward":
                          # covers the case where both lines are visible at once).
                          # TUNE ON MAT: turn still clips the inner wall -> raise;
                          # turn starts too late / overshoots the zone -> lower.
CONFIRM_TIMEOUT_S = 2.0   # confirmation colour never seen (missed / occluded /
                          # bad HSV) -> fire the turn anyway so one missed line
                          # can never strand the run mid-corner. Must stay well
                          # under fsm.CORNER_COOLDOWN_S + a straight transit.
...
class VisionPipeline:
    def __init__(self, on_command=None, camera_index=CAMERA_INDEX,
        ...
        self._last_dir = 0
        self._armed = True
        # two-line corner sequence (stage path): "watch" = looking for the ENTRY
        # line; "confirm" = entry registered, driving between the lines, waiting
        # for the confirmation colour (or the timeout) to fire the turn.
        self._corner_phase = "watch"
        self._entry_t = 0.0
    ...
    def process(self, frame):
        ...
        if line_present and self.turn_direction == 0:     # CAMERA authority: the FIRST
            self.turn_direction = ORANGE_TURN if o >= b else BLUE_TURN   # colour latches CW/CCW

        # TWO-LINE corner sequence (see the CONFIRM_* block above). The colour that
        # latched the direction is the ENTRY colour for the whole run (CW: orange,
        # CCW: blue); the OTHER colour is the confirmation. The entry line only
        # REGISTERS the corner -- the turn fires from BETWEEN the lines, when the
        # confirmation colour shows up (or on the timeout fallback).
        fire = 0
        if self.turn_direction != 0:
            if self.turn_direction == ORANGE_TURN:        # CW: orange entry, blue confirms
                entry_px, entry_row, confirm_px = o, o_row, b
            else:                                         # CCW: blue entry, orange confirms
                entry_px, entry_row, confirm_px = b, b_row, o
            if self._corner_phase == "watch":
                # entry line must sit LOW in the frame (near the nose) -- same
                # LINE_FIRE_ROW gate as before, now it arms instead of turning
                near = entry_px >= LINE_MIN_PIX and entry_row >= LINE_FIRE_ROW
                if self._latch_corner(self.turn_direction if near else 0):
                    self._corner_phase = "confirm"        # registered, NOT fired
                    self._entry_t = time.time()
                    print(f"[vision] entry line registered (o={o} b={b} "
                          f"row={entry_row}) -> driving between the lines", flush=True)
            else:                                         # "confirm"
                waited = time.time() - self._entry_t
                if confirm_px >= CONFIRM_MIN_PIX and waited >= CONFIRM_MIN_S:
                    fire = self.turn_direction
                    print(f"[vision] confirmation line seen ({confirm_px}px after "
                          f"{waited:.2f}s) -> turn({fire})", flush=True)
                elif waited >= CONFIRM_TIMEOUT_S:
                    fire = self.turn_direction            # missed line can't strand the run
                    print(f"[vision] NO confirmation line after {waited:.2f}s "
                          f"-> turn({fire}) on timeout", flush=True)
                if fire:
                    self._corner_phase = "watch"          # re-arms via the clear streak

        if max(o, b) > LINE_MIN_PIX // 2:                 # surface it while tuning
            cmd.note = f"{cmd.note} | line o={o} b={b} phase={self._corner_phase}"
        cmd.turn = fire
        self.last = cmd
        self._overlay = (red, green, walls, cmd)
        return cmd, red, green, walls
